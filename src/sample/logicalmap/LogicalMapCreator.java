...
public class LogicalMapCreator {
 ...
 //       putEnemiesOnLogicalMap(generateEnemyStartingCoordinates(numberOfEnemies, width, height), logicalMap, illegalLocations); // TODO: 12/6/2020 add number of enemies
    ...
    public void putCharacterOnLogicalMap(Pair<Integer, Integer> coordinate, LogicalMap logicalMap) {
    ...
    /*
        // TODO: 12/6/2020 check if this works
        public void putEnemiesOnLogicalMap(Set<Pair<Integer, Integer>> coordinates, LogicalMap logicalMap, Set<Pair<Integer, Integer>> illegalLocations) {
            ...
            for (Pair<Integer, Integer> coordinate : coordinates) {
                if (!illegalLocations.contains(coordinate)) {
                    positions[coordinate.getKey()][coordinate.getValue()].setEnemy(new Enemy(logicalMap, new Pair<>(coordinate.getKey(), coordinate.getValue())));
                }
            }
        }

        // TODO: 12/6/2020 and if this works
        public Set<Pair<Integer, Integer>> generateEnemyStartingCoordinates(int number, int width, int height) {
            ...
            while (coordinates.size() < number) {
                ...
                int x = (int) (Math.random() * (width - ((width / 10)) - 1));
                assert x >= 0;
                assert x < width;

                int y = (int) (Math.random() * (height - ((height / 10)) - 1));
                assert y >= 0;
                assert y < height;
